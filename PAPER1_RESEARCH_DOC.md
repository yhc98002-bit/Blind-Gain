...
**Landed 2026-08-07: R4 complete — THE LADDER IS CLOSED (R1–R5).** The registered 7B readout (`reports/c5_r4_readout_v1.*`, all 18 checks true) delivers the strongest form of the scale claim: A1's matched gain replicates almost exactly across scales (+0.2479 at 7B vs +0.2435 at 3B — the recipe transfers), while the gray-trained arm's crossed TrainShare rises from 0.487 [0.383, 0.588] pooled at 3B to **0.7785 [0.6418, 0.9214]** canonical / **0.8402 [0.7457, 0.9456]** strict at 7B — disjoint intervals; **the access phenomenon grows with scale**. Matched A2-gray gain stays small (+0.0516), so F1's two-regime matrix reproduces at 7B with a wider crossed/matched gap. One seed; cross-scale contrast descriptive. Collected in RESULTS §12e.

**Landed 2026-08-11: C6 — the mechanism at 7B. F3's layer selectivity is scale-conditional, and it inverts.** C6 joins the program's two one-axis results: Gate 1 fixed scale (3B) and varied recipe (four recipes, no axis buys held-out content on the primary anchor, the oracle-localized readout moves under every one); R4 fixed recipe and varied scale. Six cells — frozen 7B base / C5 A1-real / C5 A2-gray × R19 and the R20 private twin — registered pre-read (`docs/registered_c6_mechanism_at_scale_v1.md`), all 16 acceptance checks passing, fixtures green first.

The recipe-matched **A1-real** arm fires registered branch **(d): the primary visual anchor MOVES** — `coordinate_register_twenty_point_x_v02` **+0.0250 [0.0033, 0.0467]** lenient and **+0.0250 [0.0050, 0.0467]** strict on R19, replicated **+0.0233** on the item-disjoint R20 twin under both contracts — **while the oracle-localized readout does not**. The blind-trained **A2-gray** arm, same recipe and steps, fires **(c) neither MOVED**. The canary is clean everywhere. This is the first result in the program where an RLVR arm moves the primary visual anchor on a held-out counterfactual instrument at all, and the A2-gray control is what makes it a *content* statement rather than a format-or-competence one: the movement required real pixels.

**Consequence for §3's mechanism paragraph (PI-owned wording).** F3 currently reads "the gain lands exactly where the visual work has been done for the model" — established at 3B and reproduced there under four recipes. C6 does not contradict that; it dates it. At 7B the same recipe reaches the layer that 3B could not, so the correct statement is that *which* layer is reachable depends on what the encoder already supports, and the 3B ordering is a property of 3B rather than of outcome reward. §3 and §5 need a scale clause; the numbers to carry it are in `reports/c6_mechanism_at_scale_v1.*`. Read alongside R4 the two results are complementary, not in tension: on the *training* distribution the blind-attainable share grows with scale, while on a *held-out counterfactual* instrument the same 7B pair separates the arms in the opposite direction — only the sighted arm moves the anchor.

Scope, stated with the claim: one seed, one 7B training pair, two arms not four; the anchor effect is small in absolute terms (+2.3 to +2.5 points against a 0.757–0.785 base) and its interval clears zero without a wide margin — what carries it is the four-way replication across an item-disjoint twin and both contracts while the blind arm's does not move. Branch (d) was named in the registration precisely so it could not be folded into a friendlier branch, and it carries no pre-committed interpretation; C6 re-decides neither Gate 1 nor R4 and computes no cross-scale statistic. Independently reproduced by a second instrument written against the registration alone: 24/24 registered numbers exact. Collected in RESULTS under the 2026-08-11 C6 section.

**Still pending:** M7 seed 2 (upgrade to the two-seed estimator; a1 training, remaining arms queued). LH2 stage 1 (registered, chain armed). Human gates; X6 related-work table (PI-owned).
