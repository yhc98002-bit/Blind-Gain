...
| 2026-08-04T00:45Z (logged) | c5_a1_real_seed1_7b attempt 1 (`c5_a1_real_seed1_7b_an29_20260801T045537Z`) | Killed by Ray's host-memory monitor at ~step 81 of 100 (2026-08-03T16:42Z): `ray.exceptions.OutOfMemoryError: Task was killed due to the node running low on memory`. GPU memory was never exceeded. Proximate cause: a second trainer (M7 `a1_real` seed 2, 3B) was launched onto the same node (an29 GPUs 0-3) at 15:52Z while this arm's fp32 parameter+optimizer offload held a large host-RAM footprint; both trainers died in the same cascade ~50 min later. | Orchestrator placement error, not a config or recipe defect: the arm had trained stably for 2.5 days and 80 steps before the colocation. | **None on any estimand** — the arm never reached `step_final`, no value was read, and `save_model_only` checkpoints cannot resume, so attempt 1 contributes nothing. Its checkpoints are archived at `checkpoints/c5/c5_a1_real_seed1_7b_attempt1_hostoom_20260803` as evidence. Attempt 2 relaunched 2026-08-04T00:41Z as a fresh run of the **same immutable config** (sha unchanged, launcher gates re-passed) on an29 GPUs 4-7. **New placement rule adopted: no second trainer is colocated on a node running a 7B host-offload arm** — CLAUDE.md's 'colocation is normal' holds for disjoint-GPU jobs, but host RAM is a shared resource the GPU-scope guard does not see. | Logged. |
| 2026-08-06T03:43Z (logged, pre-completion) | Readout instrumentation (no training unit) | During schema reconnaissance for the registered readout script (`scripts/build_c5_r4_readout.py`), a `uniq -c` over the two scoring-contract booleans (`greedy_canonical_correct`, `greedy_acc_strict`) of the base-real test split was run, making the two base-cell contract marginal counts visible to the executing agent before the training arms completed. | Schema confirmation of I7 field divergence (the two contracts are distinct fields that disagree on real rows), needed to bind the readout script to the correct field names before the arm data exists. | None on any estimand: the base cells are frozen inference-only evaluations, no arm value exists yet, no gain or recovery can be formed without the arms, and no estimand was computed. The registered readout script refuses all estimands until all six cells are bound (`--partial` is values-free by design; verified against both base cells with zero performance values emitted). Logged before any value is interpreted per this log's discipline. | Logged. |
