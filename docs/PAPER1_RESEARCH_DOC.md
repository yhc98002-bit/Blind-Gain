...
**Landed 2026-08-11: C6 — the mechanism at 7B, and it inverts.** Gate 1 fixed scale and varied recipe (no axis buys held-out content on the primary visual anchor; the oracle-localized readout moves under every recipe). C6 fixes recipe and varies scale, on the banked C5 7B checkpoints: six cells, three models × R19 + the R20 private twin, registered before any value was read, 61 adversarial fixtures green first, all 16 acceptance checks pass (`reports/c6_mechanism_at_scale_v1.*`, `docs/registered_c6_mechanism_at_scale_v1.md`). **A1-real moves the primary visual anchor** — +0.0250 [0.0033, 0.0467] on R19, +0.0233 [0.0017, 0.0433] on R20, both contracts, registered branch **(d)** replicating 4-way — **while the oracle-localized readout does not move**. **A2-gray moves neither role** on either instrument (branch (c)), so the anchor movement is image-dependent: same recipe, same steps, real pixels the only difference. This is the first arm in the program to move the primary anchor at all. One seed, one 7B pair; the effect is +2.3–2.5 points on a 0.757–0.785 base; branch (d) carries no pre-committed interpretation, so the reading is descriptive and C6 re-decides neither Gate 1 nor R4. Collected in RESULTS (2026-08-11 section).

**Still pending:** M7 seed 2 (upgrade to the two-seed estimator; a2_gray/a3 training, evals queued). LH2 stage 1 (registered, chain armed). Human gates; X6 related-work table (PI-owned).
