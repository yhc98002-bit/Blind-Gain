...
| 2026-08-10T14:40Z (logged) | m7_virl_a2_gray_seed2 attempt 1 (`m7_virl_a2_gray_seed2_an12_20260809T145133Z`) | Killed at ~step 20+/100 by Ray's host-memory monitor (07:57:37Z): `ray.exceptions.OutOfMemoryError: Task was killed due to the node running low on memory`. Colocated with the LH2 seg-1 trainer (an12 0-3); both logs end the same second — one node-level RAM exhaustion took both. | Orchestrator placement error: two host-RAM-ramping Ray trainers on one node. The 2026-08-03 rule covered only 7B host-offload arms; measured ramp (~16 GiB/h/trainer, spikes at checkpoint saves) shows two 3B ViRL trainers also exceed a 1007-GiB node within ~17 h. | **None on any estimand** — no value read; checkpoints archived `checkpoints/m7/m7_virl_a2_gray_seed2_attempt1_hostoom_20260810`. Attempt 2 relaunched 14:39Z same immutable config, **solo on an12 4-7**. **Placement rule generalized: one ramping ViRL/GRPO trainer per node; small 1-GPU evals may colocate; host RAM is shared and invisible to the GPU-scope guard.** | Logged. |
| 2026-08-10T14:40Z (logged) | m7_virl_a3_caption_seed2 attempt 1 (`m7_virl_a3_caption_seed2_an29_20260809T235030Z`) | **Preemptively stopped by operator** (SIGKILL, exit 137, ~step 25/100): an29 carried the same two-trainer colocation (a2b + a3) that had just killed both an12 jobs; projected exhaustion (~550 GiB avail / ~32 GiB/h) fell before a2b's completion. Younger job stopped to preserve a2b (step 60+). | Same placement-rule violation, caught before the OOM. | **None on any estimand** — no value read; checkpoints archived `checkpoints/m7/m7_virl_a3_caption_seed2_attempt1_hostram_20260810`; manifest closed with explicit exit-code provenance. Relaunch queued solo on an29 4-7 after a2b completes (`scripts/seed2_an29_chain.sh`); backstop watchdog `scripts/host_ram_watchdog.sh` kills the youngest trainer if avail < 120 GiB with >1 trainer. | Logged. |
