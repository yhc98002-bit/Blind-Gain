...
*Executor result 2026-08-11 — C6 puts a scale coordinate on the same axis (`reports/c6_mechanism_at_scale_v1.*`).* At 7B, on the same held-out FlipTrack instruments, the ordering that Gate 1 established at 3B **inverts**: the real-image arm moves the **primary visual anchor** (+0.025 R19 / +0.023 R20, both contracts, replicated on an instrument sharing no items) and leaves the oracle-localized readout flat, while the blind-trained arm moves neither. So "no axis buys held-out content" is a statement about 3B, not about outcome-reward RLVR as such — at 7B a plain outcome reward does reach the anchor, and only when the images are real. For this paper that narrows the target: the premise-v2 construct is still the instrument of record for *resolvability*, but Stage 3 is no longer a scale-up formality — it is where the mechanism the method is designed to force may already be partly available for free.

**Track-4 premise-v2 acceptance gates (2026-08-11).** E4 PASS (no transferable artifact signal; folded gate statistic ≤0.546, CI upper ≤0.576). E1 FAIL, registered branch **(c) still too hard** — the n=20→n=8 lever moved premise solvability 0.275→0.2875, i.e. the difficulty is not the candidate-set size, so the pre-committed step to n=5 should be taken for diagnosis more than for effect. E2: the **premise clause passes everywhere at blind accuracy exactly 0.000** — the construct is blind-unsolvable as designed — while the **final clause fails for all five types** (blind final member accuracy 0.1375–0.250 vs the 0.133 ceiling) via a degenerate constant answer (blind pair accuracy 0.000, collapse rate 1.000) meeting a non-uniform gold distribution. That is an answer-balance property of the generator, not a visual leak; the failing types stay out of training use until the final-answer distribution is balanced, and the premise construct does not need regenerating. E3 caption stress running. `reports/track4_premise_v2_gate_readout_v1.*`, `reports/track4_premise_v2_attacker_gate_v1.json`.
