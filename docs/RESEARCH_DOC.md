...
- M2: A1 recovery, A2b retry4, and A3 resume20 training manifests are complete. Their exact step-100 R19 lifecycles are also complete, each with all 15 structural marker checks true; performance values remain unopened. A2 retry2 `mech_a2_gray_resume60_retry2_an12_20260715T165701Z` continues from the original audited step-60 state on `an12:0-3` and is mechanically at step 69. Locked 601-row Geometry3K-test readouts for A1, A2b, and A3 run as independent TP1 jobs on `an12:4-6` from implementation commit `0183937`; each binds its training manifest, retained merged-checkpoint hash, prompt/parser/reward versions, and greedy decoding contract. The temporary `an21` attempt remains finalized fail with no durable checkpoint and its uncheckpointed steps excluded.
...
1. **M2/M3** — complete and audit the three active A1/A2b/A3 Geometry3K readouts, finish isolated A2 retry2 plus its retention/R19/Geometry3K lifecycle, compute the registered seed-1 estimands, then run seeds 2-3 with the hurdle mechanism analysis.
