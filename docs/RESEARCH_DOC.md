...
- M2: A2b retry4 and A3 resume20 remain active on an29. Original A1 and A2-gray failed together after their durable step-60 saves when two colocated arms drove an12 to 957.25/1007.52 GB host RAM, crossing Ray's 0.95 kill threshold; this was not CUDA OOM or storage exhaustion. A1 recovery `mech_a1_real_resume60_an12_20260714T080855Z` runs alone on an12 from a 13-file SHA256-audited step-60 state. The newly assigned an21 passed eight-A800, host-memory, shared-path, `/dev/shm`, and Ray-path preflights; A2 recovery `mech_a2_gray_resume60_an21_20260714T145532Z` is launched there alone from its independently audited step-60 state. The old lifecycle queue was retired before direct launch. A 24-GPU three-node monitor and replacement four-arm completion watchdog are active. No pilot performance metric was inspected.
...
1. **M2/M3** — validate isolated A1/an12 and A2/an21 recoveries, complete and audit seed 1 across the three nodes, then run seeds 2-3 with the registered hurdle mechanism analysis and checkpointed FlipTrack readouts.
