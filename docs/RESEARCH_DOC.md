...
- M2: A1, A2b, and A3 training plus exact step-100 R19 lifecycles are complete. Their locked 601-row Geometry3K-test readouts are also complete and independently audited: source/checkpoint hashes, all row identities, every recomputed score, and strict accounting pass with zero mismatches; performance values remain unopened. A2 retry2 `mech_a2_gray_resume60_retry2_an12_20260715T165701Z` is mechanically at step 70 on `an12:0-3`. Its R19 queue reports `waiting_training`; a second committed queue reports `waiting_r19_marker` and will launch the exact 601-row Geometry3K readout plus an independently bound audit only after R19 completes. Both are CPU-only while waiting. The released `an21` node is rejected by the queue validator; its old temporary attempt remains finalized fail with no durable checkpoint and its uncheckpointed steps excluded.
- M8: the shared 7B snapshot was staged to `an29:/dev/shm` with per-file SHA256 verification. Real/gray/no-image/own-caption resumed from their exact 118/126/118/126 batch-aligned prefixes on GPUs 0-3, and noise started fresh on GPU 4. All five independent TP1 jobs continue advancing. A committed CPU-only login queue binds their exact identities and will aggregate plus independently recompute every score only after all five reach 4,096 rows; partial performance values remain unopened.
...
- M12: the declared 100-pair v08 calibration batch remains immutable. Structurally complete, still-uninterpreted lifecycles exist for 3B/7B real, caption, gray, and noise cells, the 7B no-star/random-star necessity diagnostic, and grouped DINOv2/frequency/metadata attackers. The opaque human package remains ready for a no-zoom audit. A guarded CPU/network-only ModelScope download of Qwen2.5-VL-72B runs on `an12` into `/dev/shm`; its declared 160 GB budget leaves about 249 GiB projected free and allocates no GPU. TP4 serving remains subordinate to M2, and human, strong-caption scoring, interpretation, freeze, and confirmatory gates remain.
...
1. **M2/M3** — preserve the three audited A1/A2b/A3 Geometry3K readouts; let the committed A2 retention, R19, and independently audited Geometry3K queues finish in order on `an12`; compute the registered seed-1 estimands only after the fourth audit, then run seeds 2-3 with the hurdle mechanism analysis.
...
3. **M8** — monitor the five active 7B pilot-contract blind-solvability conditions; let the committed completion queue aggregate and independently audit the exact 4,096-item artifacts, then fill only the audited flagship fields.
...
7. **M11/M12** — monitor the active full-only M11 watchdog; it remains GPU-inert until all four M2 step-100 markers validate, then schedules the 18 full cells on `an29`. Verify the active 72B v08 download, complete the 100-pair no-zoom human audit, then run TP4 strong-caption scoring only after M2 releases a four-GPU block. M13 maintenance consumes only landed, hash-pinned readouts.
