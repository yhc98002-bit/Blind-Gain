...
- M2: A2b retry4 and A3 resume20 remain active on an29. Original A1 and A2-gray failed together after their durable step-60 saves when two colocated arms drove an12 to 957.25/1007.52 GB host RAM, crossing Ray's 0.95 kill threshold; this was not CUDA OOM or storage exhaustion. A1 recovery `mech_a1_real_resume60_an12_20260714T080855Z` now runs alone on an12 from a 13-file SHA256-audited step-60 state, with a 650-GiB `MemAvailable` launch floor. A2's eight raw shards were guard-restored and verified, then placed behind a lifecycle-only queue that waits for a fully released node. A new one-hour monitor covers all 16 GPUs, host memory/swap, user RSS, process survival, logs, and progress. No pilot performance metric was inspected.
...
1. **M2/M3** — validate isolated A1 recovery, let the release queue launch A2 on the first fully free node, complete and audit seed 1, then run seeds 2-3 with the registered hurdle mechanism analysis and checkpointed FlipTrack readouts.
