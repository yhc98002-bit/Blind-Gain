...
- M11: ModelScope-first, per-file-verified stages for InternVL3-9B and Gemma-3-12B-IT remain in an29 `/dev/shm`. The complete R19/R20 and 4,096-item ViRL adapters are committed, and immutable queue `m11_generalization_queue_login_20260713T044555Z` is waiting on all four M2 manifests; six smoke cells fail closed before the 18-cell full matrix.
