...
- M11: the pre-existing marker-only watchdog opened when A2's exact R19 marker landed and launched eight InternVL cells on an29 before the seed-1 report completed. Two ViRL blind cells failed quickly, two caption cells completed, and four FlipTrack cells continue on GPUs 0/1/3/4. They are left undisturbed. Ten cells remain pending behind M5 and seed-2 placement. The EasyR1 environment is unchanged.
...
2. **M11** — preserve the six active InternVL FlipTrack cells, diagnose the two failed blind cells, and launch the ten remaining cells only after M5/seed-2 placement.
