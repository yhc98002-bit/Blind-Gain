...
- M2: A1 recovery, A2b retry4, and A3 resume20 training manifests are complete. The temporary `an21` allocation was released after A2 logged steps 61-64 but before any new checkpoint; that attempt is finalized fail, has no durable resume state, and its uncheckpointed work is excluded. A2 retry2 `mech_a2_gray_resume60_retry2_an12_20260715T165701Z` now runs alone on `an12:0-3` from the original 13-file SHA256-audited step-60 state. A 16-GPU two-node health monitor and a replacement four-arm completion watchdog are active; the watchdog reports 3/4 complete. A1/A2b/A3 registered step-60 R19 evaluation, aggregation, and fail-closed marker lifecycles are complete without operational metric inspection; A2b/A3 retention has resumed toward steps 80/100, and A1 retention recovery is queued behind the active quota scans.
...
- M11: the retained V1 queue opened after the blind arms completed and finalized fail with three Gemma smoke completes, three InternVL smoke failures, and all 18 full cells pending. Root cause was a missing pinned `einops` dependency plus a verifier that never imported InternVL's local dynamic model class. Runtime V2 now passes exact package, Gemma, and real `InternVLChatModel` import checks; fresh InternVL smoke cells are next. The EasyR1 environment is unchanged.
...
1. **M2/M3** — complete and audit the isolated A2 retry2 on an12, finish step-60/100 FlipTrack and geo3k readouts plus checkpoint retention for all four arms, then run seeds 2-3 with the registered hurdle mechanism analysis.
...
7. **M11/M12** — rerun the three failed InternVL smoke cells against runtime V2, then schedule the full non-Qwen matrix behind M2 checkpoint evaluations; chart v08 fills later safe capacity, and M13 maintenance consumes only landed, hash-pinned readouts.
