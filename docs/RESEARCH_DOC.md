...
7. **M11/M12** — monitor the active full-only M11 watchdog; it remains GPU-inert until all four M2 step-100 markers validate, then schedules the 18 full cells on `an29`. Chart v08 fills later safe capacity, and M13 maintenance consumes only landed, hash-pinned readouts.
