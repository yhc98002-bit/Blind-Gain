...
- M2: A1 recovery, A2b retry4, and A3 resume20 training manifests are complete. The temporary `an21` allocation was released after A2 logged steps 61-64 but before any new checkpoint; that attempt is finalized fail, has no durable resume state, and its uncheckpointed work is excluded. A2 retry2 `mech_a2_gray_resume60_retry2_an12_20260715T165701Z` runs alone on `an12:0-3` from the original audited step-60 state and is mechanically at step 66. A one-hour 16-GPU monitor found no fatal pattern, at least 657.9 GiB host headroom, and zero swap. A2b serialized retention and its registered step-100 R19 lifecycle are complete. A3 retention is complete; its hash-pinned queue waits for two clean capacity polls on `an29:4-7`. The serialized handoff launched A1 retention, and a separately committed queue waits for that exact retention lifecycle before using `an12:4-7`.
- M8: the shared 7B snapshot was staged to `an29:/dev/shm` with per-file SHA256 verification. Real/gray/no-image/own-caption resumed from their exact 118/126/118/126 batch-aligned prefixes on GPUs 0-3, and noise started fresh on GPU 4. All five independent TP1 jobs passed the first health check; completion, aggregation, and audit remain.
...
- M12: the declared 100-pair v08 calibration batch remains immutable. A V2 CPU audit reconstructs every image, verifies exact masks/mechanics and CVD-aware dual coding, and adds 400 answer-discordant member-level no-star/randomized-star images. A separate 200-row necessity-evaluation manifest is fixed to original member answers. The opaque 100-pair human package passes all linter checks and is ready for a no-zoom audit. Structurally complete, still-uninterpreted lifecycles now exist for 3B/7B real, 3B/7B question-blind caption QA, the 7B no-star/random-star necessity diagnostic, and the grouped DINOv2/frequency/metadata attackers; human, strong-caption, attacker/necessity interpretation, freeze, and confirmatory gates remain.
...
1. **M2/M3** — complete and audit the isolated A2 retry2 on an12, let the A3 capacity queue and A1 retention-bound queue finish their registered step-100 FlipTrack lifecycles, finish geo3k readouts for all four arms, then run seeds 2-3 with the registered hurdle mechanism analysis.
...
3. **M8** — monitor the five active 7B pilot-contract blind-solvability conditions, then aggregate and audit the complete 4,096-item preparation artifact.
...
7. **M11/M12** — monitor the active full-only M11 watchdog; it remains GPU-inert until all four M2 step-100 markers validate, then schedules the 18 full cells on `an29`. Complete chart-v08's 100-pair no-zoom human audit, strong-caption stress, attacker gate, and necessity-effect analysis on later safe capacity. M13 maintenance consumes only landed, hash-pinned readouts.
