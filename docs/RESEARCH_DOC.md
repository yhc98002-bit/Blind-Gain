...
- M2: A1 recovery, A2b retry4, and A3 resume20 training manifests are complete. The temporary `an21` allocation was released after A2 logged steps 61-64 but before any new checkpoint; that attempt is finalized fail, has no durable resume state, and its uncheckpointed work is excluded. A2 retry2 `mech_a2_gray_resume60_retry2_an12_20260715T165701Z` runs alone on `an12:0-3` from the original audited step-60 state. A one-hour 16-GPU monitor observed step advance to 61, about 70-72% mean utilization per assigned GPU, no fatal pattern, at least 657.9 GiB host headroom, and zero swap. A1/A2b/A3 registered step-60 R19 lifecycles are mechanically complete. Concurrent post-step80 quota scans failed A2b/A3 retention after successful merges; a new serialized, hash-pinned A2b step80/100 recovery runs before A3 and A1.
...
- M11: the retained V1 queue opened after the blind arms completed and finalized fail with three Gemma smoke completes, three InternVL smoke failures, and all 18 full cells pending. Runtime V2 fixed the missing pinned `einops` dependency and added a real local class import. A second retained smoke generation exposed dynamic-versus-legacy cache incompatibility; commit `8a5525b` makes the shim select legacy caches. Fresh InternVL real/no-image/caption cells now complete with required artifacts, mechanically covering all six smoke cells. The full matrix remains pending behind M2 evaluation priority, and the EasyR1 environment is unchanged.
...
1. **M2/M3** — complete and audit the isolated A2 retry2 on an12, serialize A2b/A3/A1 retention recovery, finish step-100 FlipTrack and geo3k readouts for all four arms, then run seeds 2-3 with the registered hurdle mechanism analysis.
...
7. **M11/M12** — six M11 smoke cells are mechanically covered; build a full-only recovery queue and schedule it behind M2 checkpoint evaluations. Chart v08 fills later safe capacity, and M13 maintenance consumes only landed, hash-pinned readouts.
