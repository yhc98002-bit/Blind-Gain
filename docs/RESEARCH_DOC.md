...
- M2: A1, A2-gray, and A3 remain active. A2b retry1 failed pre-checkpoint from CUDA fragmentation; retry2 deadlocked pre-checkpoint after compute-`/tmp` inode exhaustion and was terminated after a seven-sample 0%-GPU liveness audit. Retry3 is prepared with unchanged scientific config, expandable segments, and job-local `/dev/shm` temp routing. No pilot performance metric was inspected.
