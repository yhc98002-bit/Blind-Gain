...
- M11: A2b's vacancy triggered four smoke attempts; InternVL exposed a null nested generation config and Gemma visual masking requires Torch >=2.6. The full matrix stayed closed. Tracked fixes plus a separate `.venv-m11` design pass focused tests; the first setup failed before download because system `venv` lacks ensurepip, and an explicit `virtualenv` retry is prepared without touching the pilot environment.
