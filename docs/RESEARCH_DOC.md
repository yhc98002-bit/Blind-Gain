...
- M2: all four registered A1/A2/A2b/A3 training runs reached step 100 and exited successfully. A1, A2b, and A3 exact step-100 R19 lifecycles plus locked 601-row Geometry3K readouts are complete and independently audited with zero identity, score, or strict-accounting mismatches; their values remain unopened. A2 raw state is intact, but the old retention watcher failed when login scratch could not preserve its 40-GiB floor; its dependent queues failed without evaluation. PI-approved recovery is active: a superseded A3 archive is being hash-preserved on shared storage, the A2 step-100 merge runs independently under R3, and new immutable R19 then Geometry3K/audit lifecycles will follow. No retraining is indicated.
...
- M11: three Gemma and three repaired InternVL smoke cells are complete and hash-pinned. The committed full-only login watchdog is active with 18/18 cells pending and zero child GPU jobs; valid step-100 FlipTrack markers from all four M2 arms are a hard precondition for any `an29` allocation. The EasyR1 environment is unchanged.
...
- M10: the support-sharpening protocol and fail-closed adapter are registered. Eligibility is fixed to the arm's own 0/16 baseline items that change from step-0 greedy wrong to step-100 greedy correct; exactly 64 frozen-base follow-up samples and non-causal wording are required. Candidate execution waits for audited post-training rows.
...
1. **M2** — complete the A2 step-100 merge, fresh immutable R19 lifecycle, 601-row Geometry3K audit, and the single locked four-arm seed-1 readout; three-arm values stay unopened.
2. **M3 seed 2 + M5** — launch seed-2 arms and the fixed step-400 anchor continuation concurrently after M5 restore integrity passes.
3. **M11** — run the full 18-cell matrix after A2 creates the fourth exact M2 marker.
4. **M6** — launch the matched CP-versus-member-reward control when the pair corpus and reward/grouping audits are ready.
5. **M3 seed 3** — launch the third pilot seed after the higher-priority seed-2/M5/M11/M6 work is placed.
6. **M7/M9** — run the stratified ViRL 3B decomposition and four-arm, three-seed 7B flagship according to their frozen data/config readiness.
7. **M10** — fold 64-sample support-sharpening checks into every applicable readout.
8. **M12/M13** — fixed-size no-zoom v08 viewer is ready; Richard schedules human review after seed 1. Run TP4 strong-caption scoring when priority work releases four GPUs. Maintain paper slots only from landed, hash-pinned readouts.
9. **M14** — 7B CP merge-back readouts only after mini-A5 and flagship seed 1.
