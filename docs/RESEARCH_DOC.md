...
- M2: A1, A2-gray, and A3 remain active. A2b retry1 failed pre-checkpoint from CUDA fragmentation; retry2 deadlocked pre-checkpoint on compute-`/tmp`; retry3 then failed closed before process startup because its launch lock still used that full filesystem. Retry4 is prepared with unchanged scientific config and all ephemeral runtime/lock paths under `/dev/shm`. No pilot performance metric was inspected.
