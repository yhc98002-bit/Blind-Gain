...
- M11: ModelScope-first stages for InternVL3-9B and Gemma-3-12B-IT remain verified in an29 `/dev/shm`. Both architectures construct at full parameter counts under meta weights; a tracked, idempotent runtime shim restores the nested InternVL `GenerationMixin` removed by Transformers 4.56, without modifying model source. Capacity queue `m11_generalization_queue_login_20260713T050649Z` still requires two stable free-GPU polls and six smoke cells before the 18-cell matrix.
