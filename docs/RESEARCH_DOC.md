...
- M5/M3: all eight step-100 anchor model/optimizer shards were restored and hash-audited. The one-step integrity run is active on an12 GPUs 0-3; its passing machine result will trigger the already queued terminal step-400 continuation. Seed-2/3 configs are merged and hash-registered. The seed-2 scheduler reserves the M5 node and may use one four-GPU placement on the other node; it never reads performance logs.
...
- Storage: the post-sweep quota-aware 2026-07-16 snapshot records 915,453,743,104 bytes used and 695,158,992,896 bytes available under the 1.5-TiB allocation. The explicitly superseded 49,101,916,160-byte initial A3 archive was listed and hashed before its verified preservation move to shared storage; the source is now a symlink to the preserved destination and unrelated `/tmp` content remains untouched.

## 6. Upcoming experiments (order; ~60–70 node-days vs ~120 available)

1. **M3 seed 2 + M5** — complete the active anchor one-step integrity check, then let the queued fixed step-400 continuation occupy its reserved node while seed-2 arms use at most one trainer on the other node.
2. **M11** — monitor the three corrected InternVL blind-sample cells and four original FlipTrack cells to completion without opening performance values, reconcile the complete InternVL matrix, then launch the nine queued Gemma cells as capacity permits.
3. **M6** — launch the matched CP-versus-member-reward control when the pair corpus and reward/grouping audits are ready.
