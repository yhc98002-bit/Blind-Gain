...
- M11: ModelScope-first, per-file-verified stages for InternVL3-9B and Gemma-3-12B-IT remain in an29 `/dev/shm`. Both complete model-class import paths pass no-GPU local-files-only preflight after pinning official InternVL dependency `timm==0.9.12`; Gemma's slow processor is explicit. Capacity queue `m11_generalization_queue_login_20260713T050649Z` requires two stable free-GPU polls, then fails closed through six smoke cells before the 18-cell matrix.
