...
- M2: all four arms are active. A2b retry1 failed before any checkpoint from CUDA allocator fragmentation; retry2 (`mech_a2b_noimage_retry2_an29_20260713T111446Z`) runs from the registered base with unchanged scientific config and `expandable_segments:True`. It continued after `/tmp` inode-exhaustion errors; future retries route Python tempfiles to job-local `/dev/shm`. No pilot performance metric was inspected.
