...
- M12: the declared 100-pair v08 calibration batch remains immutable. Structurally complete, still-uninterpreted lifecycles exist for 3B/7B real, caption, gray, and noise cells, the 7B no-star/random-star necessity diagnostic, and grouped DINOv2/frequency/metadata attackers. The 146.8-GB 72B ModelScope checkout completed and its tree hash is recorded on an12 `/dev/shm`; TP4 caption stress has not started. The viewer must mechanically disable zoom, fix rendering size, and add the two v08 ratings before Richard's post-seed-1 human audit.
...
- Storage: the quota-aware 2026-07-16 snapshot records 799.3 GiB used and 700.7 GiB available under the 1.5-TiB allocation. Login `/tmp` fell to about 45.6 GiB free because the checkpoint archive reached about 401 GiB. The explicitly superseded initial A3 archive was listed and hashed before a preservation move to shared storage; unrelated `/tmp` content is untouched.
...
8. **M12/M13** — prepare the fixed-size no-zoom v08 viewer now; Richard schedules human review after seed 1. Run TP4 strong-caption scoring when priority work releases four GPUs. Maintain paper slots only from landed, hash-pinned readouts.
