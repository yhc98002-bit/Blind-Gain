...
- M2: A1, A2b, and A3 training plus exact step-100 R19 lifecycles are complete. Their locked 601-row Geometry3K-test readouts are also complete and independently audited: source/checkpoint hashes, all row identities, every recomputed score, and strict accounting pass with zero mismatches; performance values remain unopened. A2 retry2 `mech_a2_gray_resume60_retry2_an12_20260715T165701Z` is mechanically at step 70 on `an12:0-3`. Its committed CPU-only queue reports `waiting_training` and cannot launch R19 until exact training/primary-retention completion and two clean polls on `an12:4-7`. The temporary `an21` attempt remains finalized fail with no durable checkpoint and its uncheckpointed steps excluded.
...
1. **M2/M3** — preserve the three audited A1/A2b/A3 Geometry3K readouts, finish isolated A2 retry2 plus its automated retention/R19/Geometry3K lifecycle, compute the registered seed-1 estimands only after the fourth audit, then run seeds 2-3 with the hurdle mechanism analysis.
