...
- M2: A1/A2-gray on an12 and A3 on an29 remain active. A2b failed at its first checkpoint boundary when a recursive quota-root `du` timed out before writing any checkpoint; no performance metric was inspected. A snapshot-based fail-closed guard, periodic login-node refresher, and fresh immutable retry path pass 10 focused tests and await committed relaunch on an29 GPUs 0-3.
...
- M11: ModelScope-first stages for InternVL3-9B and Gemma-3-12B-IT remain in an29 `/dev/shm`. A2b's unexpected vacancy triggered four smoke attempts, which failed before any full-matrix cell; the queue record is retained. M11 remains subordinate to pilot ownership and will be requeued only for capacity released by the blind arms.
