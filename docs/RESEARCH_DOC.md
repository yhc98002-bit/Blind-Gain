...
Layer-3 forced-choice diagnostic (registered §6; decisive, cheap) · geometry emission diagnostics (cached) · human pass on 24 expansion candidates (Richard, ~30 min, qualitative) · seeds 2–3 + M10 repeat (replication rung) · M5 step-400 (horizon rung; running, terminal rule merged) · M7 ViRL 3B stratified (dose-response rung; prediction merged pre-launch) · M8/M9 7B ×3 seeds, 4 arms (scale rung) · M6 mini-A5 two-arm (Paper-2 gate + RL positive control) · M11 non-Qwen (family rung) · chart-v08 + human audit (construct repair) · merge-back readouts.
