...
- M11: three Gemma and three repaired InternVL smoke cells are complete and hash-pinned. A new full-only recovery queue passes real-artifact preflight, contains only the 18 full cells, and is hard-gated on valid step-100 FlipTrack markers from all four M2 arms before it can allocate `an29` GPUs. It remains unlaunched until its exact code/config commit lands; the EasyR1 environment is unchanged.
...
7. **M11/M12** — launch the prepared full-only M11 login watchdog from committed HEAD; it remains GPU-inert until all four M2 step-100 markers validate, then schedules the 18 full cells on `an29`. Chart v08 fills later safe capacity, and M13 maintenance consumes only landed, hash-pinned readouts.
