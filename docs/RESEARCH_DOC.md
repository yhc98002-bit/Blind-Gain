...
- M11: the pre-existing marker-only watchdog opened when A2's exact R19 marker landed and launched eight InternVL cells on an29 before the seed-1 report completed. Two ViRL blind cells failed quickly; six FlipTrack cells remain healthy and are left undisturbed. The parent stopped fail-closed, ten cells remain pending, and its priority/scheduler state must be reconciled after M5 and seed-2 placement. The EasyR1 environment is unchanged.
...
1. **M3 seed 2 + M5** — run the anchor one-step restore integrity check, then launch seed-2 arms and the fixed step-400 continuation as single-node four-GPU placements become available.
