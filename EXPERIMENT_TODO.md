...
| **C6** | **Mechanism at scale — does the 3B readout/anchor dissociation hold at 7B?** | **Paper 1 mechanism section**; joins Gate 1 (recipe axis) to R4 (scale axis) | ✅ **complete 2026-08-11.** Six cells (frozen 7B base / C5 A1-real / C5 A2-gray × R19 + the R20 private twin, zero shared `pair_id`), registered pre-read (`docs/registered_c6_mechanism_at_scale_v1.md`, filed while cells were still generating), all 16 acceptance checks pass, adversarial fixtures green before the instrument touched a real cell. **The dissociation does not survive scale — it inverts, and only for the sighted arm.** A1-real fires registered branch **(d) anchor MOVED, readout NOT MOVED**: primary visual anchor **+0.0250 [0.0033, 0.0467]** lenient / **+0.0250 [0.0050, 0.0467]** strict on R19, replicated **+0.0233 [0.0017, 0.0433] / +0.0233 [0.0033, 0.0450]** on R20 — a 4-way replication; the oracle-localized readout is NOT MOVED on both instruments under both contracts. A2-gray (blind-trained, same recipe and steps) fires **(c) neither MOVED**, which is what makes the anchor movement a **content** statement: it required real pixels. Canary clean everywhere (no damage flag). **First result in the program where an RLVR arm moves the primary visual anchor on a held-out counterfactual instrument at all.** Scope: one seed, one 7B training pair, two arms not four; branch (d) was named in the registration precisely so it could not be folded into (a) or (b) and carries **no pre-committed interpretation** — reported descriptively; C6 re-decides neither Gate 1 nor R4, and computes no cross-scale statistic. Independently reproduced by a second instrument authored against the registration alone: **24/24 registered numbers exact**. `reports/c6_mechanism_at_scale_v1.*` (+ `_independent_replicate.*`). Collected: RESULTS 2026-08-11 C6 section. |
