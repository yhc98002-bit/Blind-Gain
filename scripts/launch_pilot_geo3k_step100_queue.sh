...
CRITICAL_FILES=(
  ...
  scripts/launch_pilot_geo3k_step100_queue.sh
...
git diff --quiet HEAD -- "${CRITICAL_FILES[@]}" || {
  echo "pilot Geometry3K queue code differs from HEAD" >&2
  exit 2
}
...
RUN_ID="pilot_geo3k_step100_queue_${ARM}_login_${STAMP}"
...
jq -n \
  ...
  '{
    ...
    run_id: $run_id,
    job_type: "pilot_geo3k_step100_evaluation_queue",
    node: "login",
    gpu_allocation: [],
    gpu_ids: [],
    tensor_parallel_width: 0,
    replica_count: 0,
    placement_justification: "CPU-only queue waits for the exact R19 marker and one free TP1 GPU, then requires a full independent audit; it sends no process signals.",
    git_hash: $git_hash,
...
kill -0 "$(cat "${RUN_DIR}/pids/login.pid")" 2>/dev/null || {
  echo "pilot Geometry3K queue exited during startup" >&2
  exit 1
}
printf '%s\n' "${RUN_DIR}"
