...
if [[ $# -ne 5 ]]; then
  echo "usage: $0 <a1_real|a2_gray|a2b_noimage|a3_caption> <an12|an29> <gpu0,gpu1,gpu2,gpu3> <retry-tag> <failed-run-dir>" >&2
  exit 2
fi
ARM="$1"
NODE="$2"
...
export BLIND_GAINS_PILOT_RECOVERY_REASON="checkpoint_guard_recursive_du_timeout_before_first_save"
