...
COMMAND="PYTHONPATH=. .venv/bin/python scripts/run_pilot_followup_queue.py --seed 2 --run-dir ${RUN_DIR} --m5-queue-state ${M5_QUEUE} --poll-seconds 60"
...
jq -n --arg run_id "${RUN_ID}" --arg git_hash "$(git rev-parse HEAD)" \
  ...
  --arg command "${COMMAND}" --arg m5 "${M5_QUEUE}" --arg start "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg log "${LOG}" --arg state "${RUN_DIR}/queue_state.json" \
  '{schema_version:"blind-gains.run-manifest.v1",run_id:$run_id,job_type:"m3_seed2_training_capacity_queue",
    ...
    m5_priority_queue_state:$m5,start_time_utc:$start,end_time_utc:null,status:"running",stdout_stderr_log:$log,
