...
run_segment() {  # n start_step target_step
  ...
  local remote_cmd="cd $ROOT && mkdir -p $run_dir/logs /dev/shm/blind-gains/${run_id}/ray && (setsid nohup env PYTHONUNBUFFERED=1 PYTHONFAULTHANDLER=1 HYDRA_FULL_ERROR=1 RAY_TMPDIR=/dev/shm/blind-gains/${run_id}/ray RAY_DEDUP_LOGS=0 CUDA_VISIBLE_DEVICES=0,1,2,3 EASYR1_ATTN_IMPLEMENTATION=sdpa HF_HOME=$ROOT/artifacts/hf_home HF_DATASETS_CACHE=$ROOT/artifacts/hf_home/datasets TRANSFORMERS_OFFLINE=1 HF_DATASETS_OFFLINE=1 PYTHONPATH=$ROOT/artifacts/repos/EasyR1 $PY -u -m verl.trainer.main config=$CONFIG trainer.max_steps=$target trainer.experiment_name=$run_id trainer.save_checkpoint_path=$STAGE_CKPT $load_arg > $run_dir/logs/an12.log 2>&1 < /dev/null & echo \$! > $run_dir/logs/pid)"
