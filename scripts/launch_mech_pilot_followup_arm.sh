...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 <2|3> <a1_real|a2_gray|a2b_noimage|a3_caption> <an12|an29> <gpu0,gpu1,gpu2,gpu3>" >&2
  exit 2
fi
...
case "${ARM}" in
  a1_real)
    CONFIG_REL="configs/train/mech_a1_real_seed${SEED}_3b_geo3k.yaml"
    ARM_RUN_NAME="mech_a1_real_seed${SEED}"
    IMAGE_CONDITION="real"
    ;;
  a2_gray)
    CONFIG_REL="configs/train/mech_a2_gray_seed${SEED}_3b_geo3k.yaml"
    ARM_RUN_NAME="mech_a2_gray_seed${SEED}"
    IMAGE_CONDITION="gray"
    ;;
  a2b_noimage)
    CONFIG_REL="configs/train/mech_a2b_noimage_seed${SEED}_3b_geo3k.yaml"
    ARM_RUN_NAME="mech_a2b_noimage_seed${SEED}"
    IMAGE_CONDITION="none"
    ;;
  a3_caption)
    CONFIG_REL="configs/train/mech_a3_caption_seed${SEED}_3b_geo3k.yaml"
    ARM_RUN_NAME="mech_a3_caption_seed${SEED}"
...
PREREG="docs/registered_pilot_seed23_v1.md"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
AUTHORIZATION="reports/pilot_followup_launch_authorization_seed${SEED}_${ARM}_${STAMP}.json"
CHECKPOINT_PATH="${ROOT}/checkpoints/pilot/${ARM_RUN_NAME}"

# This is the first executable gate and runs before any namespace or GPU action.
PYTHONPATH=. .venv/bin/python scripts/check_pilot_followup_launch_authorization.py \
  --arm "${ARM}" \
  --seed "${SEED}" \
...
if ! git ls-files --error-unmatch "${PREREG}" >/dev/null 2>&1; then
  echo "follow-up registration exists locally but is not merged in Git" >&2
  exit 2
fi
if ! git diff --quiet HEAD -- "${PREREG}"; then
  echo "follow-up registration differs from the merged Git version" >&2
  exit 2
fi
if ! git diff --quiet HEAD -- \
  ...
  scripts/prepare_pilot_followup_configs.py \
...
for GPU in "${GPUS[@]}"; do
  ...
  [[ "${USED_MIB}" =~ ^[0-9]+$ && "${USED_MIB}" -lt 1024 ]] || {
    echo "refusing pilot arm: ${NODE} GPU ${GPU} did not remain free" >&2; exit 75;
  }
done
...
if ssh "${NODE}" "pgrep -af '[p]ython.*verl.trainer.main.*BlindGain'"; then
  echo "refusing a second project EasyR1 trainer on ${NODE}" >&2
  exit 74
fi
MEM_AVAILABLE_KIB="$(ssh "${NODE}" "awk '/MemAvailable:/ {print \$2}' /proc/meminfo")"
[[ "${MEM_AVAILABLE_KIB}" -ge 681574400 ]] || { echo "follow-up pilot needs 650 GiB host memory" >&2; exit 76; }

RUN_ID="${ARM_RUN_NAME}_${NODE}_${STAMP}"
RUN_DIR="experiments/runs/${RUN_ID}"
CONFIG_SNAPSHOT="${ROOT}/${RUN_DIR}/effective_config.yaml"
PREREG_SNAPSHOT="${ROOT}/${RUN_DIR}/registered_pilot_seed23_v1.md"
...
jq -n \
  ...
  '{
    ...
    registration_snapshot: $preregistration,
    registration_sha256: $preregistration_hash,
