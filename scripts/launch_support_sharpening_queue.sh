...
CRITICAL=(
  ...
  scripts/launch_support_sharpening_queue.sh
...
git diff --quiet HEAD -- "${CRITICAL[@]}" || {
  echo "M10 queue critical code differs from HEAD" >&2
  exit 2
}
for FILE in "${CRITICAL[@]}"; do
  git ls-files --error-unmatch "${FILE}" >/dev/null 2>&1 || {
    echo "untracked M10 queue critical file: ${FILE}" >&2
    exit 2
  }
done
while IFS= read -r ACTIVE_MANIFEST; do
  if jq -e '(.job_type == "m10_support_sharpening_queue") and (.status == "running")' \
    "${ACTIVE_MANIFEST}" >/dev/null 2>&1; then
    echo "active M10 queue already exists: ${ACTIVE_MANIFEST}" >&2
    exit 73
  fi
done < <(find experiments/runs -mindepth 2 -maxdepth 2 -name run_manifest.json -type f | sort)

STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_ID="m10_support_seed1_queue_login_${STAMP}"
RUN_DIR="experiments/runs/${RUN_ID}"
MANIFEST="${RUN_DIR}/run_manifest.json"
LOG="${RUN_DIR}/logs/login.log"
STATE="${RUN_DIR}/queue_state.json"
.venv/bin/python scripts/storage_guard.py --tier S --path "${RUN_DIR}" \
  --operation m10_support_queue --required-bytes 100000000
mkdir -p "${RUN_DIR}/logs" "${RUN_DIR}/pids"
COMMAND="PYTHONPATH=. .venv/bin/python scripts/run_support_sharpening_queue.py --run-dir '${RUN_DIR}' --node an12 --allowed-gpus 5,6 --poll-seconds 60 --stable-polls 2"
...
jq -n \
  ...
  '{
    ...
    run_id: $run_id,
    job_type: "m10_support_sharpening_queue",
    node: "login",
    gpu_allocation: [],
    gpu_ids: [],
    tensor_parallel_width: 0,
    replica_count: 0,
    placement_justification: "GPU-inert queue watches only an12 GPUs 5-6 after M11 releases them; child jobs are independent single-node TP1 runs.",
    ...
    data_manifest: "47/8/7/18 frozen M10 candidate sets",
...
kill -0 "$(cat "${RUN_DIR}/pids/login.pid")" 2>/dev/null || {
  echo "M10 queue exited during startup" >&2
  exit 1
}
printf '%s\n' "${RUN_DIR}"
