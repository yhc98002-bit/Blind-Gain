...
IMAGE_INDEX="data/virl39k_main_filtered_images"
...
jq -n \
  ...
  '{
    ...
    end_time_utc: null,
    status: "running",
    expected_artifacts: [$filter, $ids, $dataset, $image_index, $freeze_summary, $report_json, $report_md]
  }' > "${MANIFEST}"
