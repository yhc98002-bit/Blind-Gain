...
case "${MODE}" in
  cp)
    SOURCE_CONFIG="configs/train/mini_a5_cp_plumbing_smoke_v1.yaml"
    EXPECTED_GROUP_MODE="joint"
    EXPECTED_REWARD_SUFFIX="src/rewards/cp_grpo_reward.py:compute_score"
    ;;
  member)
    SOURCE_CONFIG="configs/train/mini_a5_member_plumbing_smoke_v1.yaml"
...
if [[ ! "${GPU_LIST}" =~ ^[0-7](,[0-7]){7}$ ]]; then
  echo "Mini-A5 plumbing smoke requires eight comma-separated GPU indices" >&2
  exit 2
fi
IFS=',' read -r -a GPUS <<< "${GPU_LIST}"
if [[ "$(printf '%s\n' "${GPUS[@]}" | sort -u | wc -l)" -ne 8 ]]; then
  echo "Mini-A5 plumbing smoke GPU indices must be unique" >&2
  exit 2
fi

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${ROOT}"
REGISTRATION_DOC="docs/registered_mini_a5_smoke_v1.md"
REGISTRATION_MARKER="reports/mini_a5_smoke_registration_marker_v1.json"
EASYR1_DIR="${ROOT}/artifacts/repos/EasyR1-mini-a5"
PINNED_EASYR1_REVISION="dd71bbd252694f5f850213eec15795b6b88d9fea"
DATA_PATH="data/mini_a5_plumbing_val_v1.jsonl"
...
for path in "${REGISTRATION_DOC}" "${REGISTRATION_MARKER}" "${SOURCE_CONFIG}" "${DATA_PATH}"; do
...
[[ ! -e "${CHECKPOINT_PATH}" ]] || {
  echo "refusing to overwrite Mini-A5 smoke checkpoint: ${CHECKPOINT_PATH}" >&2
  exit 2
}
git diff --quiet -- \
  "${REGISTRATION_DOC}" \
...
for GPU in "${GPUS[@]}"; do
  ...
  if [[ ! "${USED_MIB}" =~ ^[0-9]+$ || "${USED_MIB}" -ge 1024 ]]; then
    echo "refusing Mini-A5 smoke: ${NODE} GPU ${GPU} has ${USED_MIB:-unknown} MiB allocated" >&2
    exit 75
  fi
done
if ssh "${NODE}" "pgrep -af '[p]ython.*verl.trainer.main.*mini_a5_.*plumbing_smoke_v1'"; then
  echo "refusing duplicate Mini-A5 plumbing smoke on ${NODE}" >&2
  exit 73
fi
...
"${ROOT}/.venv/bin/python" scripts/storage_guard.py \
  ...
  --operation "mini_a5_${MODE}_plumbing_smoke_checkpoint" \
...
jq -n \
  ...
  '{
    ...
    data_manifest: "data/mini_a5_plumbing_val_v1.jsonl",
    ...
    deviations: ["One-step engineering smoke uses 16 source prompts from the pre-frozen plumbing subset and save_model_only=true; main arms remain fixed at 400 prompts and 120 steps."]
  }' > "${MANIFEST}"
...
if ! ssh "${NODE}" "kill -0 '${REMOTE_PID}' 2>/dev/null"; then
  echo "Mini-A5 ${MODE} smoke exited during startup; inspect ${LOG}" >&2
  exit 1
fi
printf '%s\n' "${RUN_DIR}"
