...
RUN_ID="pilot_seed3_locked_eval_lifecycle_login_${STAMP}"
...
CRITICAL_FILES=(
  scripts/launch_pilot_seed3_eval_lifecycle.sh
...
CONFIG_FILES=(configs/eval/m3_seed3_*_step*_queue_v1.json)
CRITICAL_FILES+=("${CONFIG_FILES[@]}")
git diff --quiet HEAD -- "${CRITICAL_FILES[@]}" || {
  echo "seed-3 evaluation lifecycle code differs from HEAD" >&2
  exit 2
}
...
while IFS= read -r ACTIVE; do
  ...
  echo "an active seed-3 evaluation lifecycle already exists: $(dirname "${ACTIVE}")" >&2
  exit 73
done < <(find experiments/runs -mindepth 2 -maxdepth 2 -name run_manifest.json -print)

mkdir -p "${RUN_DIR}/logs" "${RUN_DIR}/pids"
printf '{"schema_version":"blind-gains.pilot-followup-eval-children.v1","seed":3,"endpoints":[]}\n' > "${CHILDREN}.partial"

ARMS=(a1_real a2_gray a2b_noimage a3_caption)
STEPS=(60 100)
for ARM in "${ARMS[@]}"; do
  for GLOBAL_STEP in "${STEPS[@]}"; do
    R19_CONFIG="configs/eval/m3_seed3_${ARM}_step${GLOBAL_STEP}_r19_queue_v1.json"
    GEO_CONFIG="configs/eval/m3_seed3_${ARM}_step${GLOBAL_STEP}_geo3k_queue_v1.json"
...
CONFIG_HASH="$(sha256sum configs/eval/m3_seed3_*_step*_queue_v1.json | sort -k2 | sha256sum | awk '{print $1}')"
...
jq -n \
  ...
  '{
    ...
    placement_justification: "CPU-only coordinator keeps all four-arm seed-3 values sealed while eight R19 endpoints serialize on four TP1 replicas and audited Geometry3K endpoints use disjoint TP1 GPUs on an29 after the training cohort drains.",
    git_hash: $git_hash,
...
kill -0 "$(cat "${RUN_DIR}/pids/login.pid")" 2>/dev/null || {
  echo "seed-3 evaluation lifecycle watcher exited during startup" >&2
  exit 1
}
printf '%s\n' "${RUN_DIR}"
