...
if [[ $# -ne 6 ]]; then
  echo "usage: $0 <retired-v1-remaining-queue> <a1-training> <a1-watcher> <a2-training> <a2-watcher> <m5-lifecycle>" >&2
  exit 2
fi
...
jq -e '(.job_type=="m3_seed3_remaining_an29_queue_v1") and
       (.status=="fail") and (.exit_code==-15)' \
  "${RETIRED_QUEUE}/run_manifest.json" >/dev/null || {
  echo "retired v1 remaining-queue identity is invalid" >&2
  exit 3
}
...
' "${RETIRED_QUEUE}/queue_state.json" >/dev/null || {
  echo "retired v1 child identities are invalid" >&2
  exit 3
}
...
jq -n \
  ...
  '{schema_version:"blind-gains.run-manifest.v1",run_id:$run_id,
    job_type:"m3_seed3_remaining_an29_queue_v2",node:"login",gpu_allocation:[],gpu_ids:[],
    ...
    deviations:["The v1 remaining-queue deadlocked: arm_checkpoint_ready required checkpoint-watcher manifest completion, but watch_pilot_checkpoints.py holds its manifest open until step-60 evaluation markers exist, and seed-3 step-60/100 evaluations run post-cohort under the registered seed-2 lifecycle procedure. arm_checkpoint_ready now releases node capacity on the artifact truth (training complete + step-100 merged index + RAW_STATE_RELOCATED) while watcher terminal failures still fail closed; step-60 raw retention closes later when the post-cohort evaluation lifecycle writes the markers.","The superseded v1 remaining queue was intentionally retired via SIGTERM after A1 reached artifact-complete state; this v2 queue adopts the same live children and the same an29-only launch restriction."]}' > "${MANIFEST}"
