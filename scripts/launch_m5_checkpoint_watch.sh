...
COMMAND="PYTHONPATH=. .venv/bin/python scripts/watch_m5_checkpoints.py --run-root ${RUN_ROOT} --archive-root ${ARCHIVE_ROOT} --run-manifest ${TRAINING_MANIFEST} --node ${NODE} --run-label m5_anchor_longhorizon_400 --expected-code-hash ${CODE_HASH} --resume-after-step ${RESUME_AFTER}"
...
jq -n --arg run_id "${RUN_ID}" --arg parent "${TRAINING_RUN}" --arg node "${NODE}" \
  ...
  --arg start "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg log "${LOG}" --argjson resume_after "${RESUME_AFTER}" \
  '{schema_version:"blind-gains.run-manifest.v1",run_id:$run_id,job_type:"m5_checkpoint_retention_watch",
    ...
    checkpoint_schedule:[150,200,250,300,350,400],resume_after_step:$resume_after,stdout_stderr_log:$log,
