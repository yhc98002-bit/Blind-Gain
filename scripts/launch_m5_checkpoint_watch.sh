...
ARCHIVE_ROOT="$(jq -r '.checkpoint_archive_root // empty' "${TRAINING_MANIFEST}")"
[[ -n "${ARCHIVE_ROOT}" ]] || ARCHIVE_ROOT="/tmp/blindgain_checkpoint_archive/${PARENT_ID}"
RESUME_AFTER="$(jq -er '.resumed_from_global_step' "${TRAINING_MANIFEST}")"
STOP_AFTER="$(jq -er '.segment_end_step // .target_global_step' "${TRAINING_MANIFEST}")"
...
COMMAND="PYTHONPATH=. .venv/bin/python scripts/watch_m5_checkpoints.py --run-root ${RUN_ROOT} --archive-root ${ARCHIVE_ROOT} --run-manifest ${TRAINING_MANIFEST} --node ${NODE} --run-label m5_anchor_longhorizon_400 --expected-code-hash ${CODE_HASH} --resume-after-step ${RESUME_AFTER} --stop-after-step ${STOP_AFTER}"
...
jq -n --arg run_id "${RUN_ID}" --arg parent "${TRAINING_RUN}" --arg node "${NODE}" \
  ...
  --arg start "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg log "${LOG}" --argjson resume_after "${RESUME_AFTER}" --argjson stop_after "${STOP_AFTER}" \
  '{schema_version:"blind-gains.run-manifest.v1",run_id:$run_id,job_type:"m5_checkpoint_retention_watch",
    ...
    checkpoint_schedule:[150,200,250,300,350,400],resume_after_step:$resume_after,stop_after_step:$stop_after,stdout_stderr_log:$log,
    expected_artifacts:[($root+"/global_step_"+($stop_after|tostring)+"/actor/RAW_STATE_RELOCATED.json")],
