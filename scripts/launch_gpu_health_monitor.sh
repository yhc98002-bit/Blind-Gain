...
RUN_ID="gpu_health_16x60m_login_${STAMP}"
...
jq -n --argjson runs "${RUNS}" --arg samples "${SAMPLES}" --arg summary_json "${SUMMARY_JSON}" --arg summary_md "${SUMMARY_MD}" '{schema_version:"blind-gains.gpu-health-monitor-config.v1",duration_seconds:3600,interval_seconds:30,runs:$runs,samples_jsonl:$samples,summary_json:$summary_json,summary_markdown:$summary_md}' > "${CONFIG}"
CONFIG_HASH="$(sha256sum "${CONFIG}" | awk '{print $1}')"
COMMAND="PYTHONPATH=. .venv/bin/python scripts/monitor_gpu_health.py --config '${CONFIG}'"
jq -n --arg run_id "${RUN_ID}" --arg git_hash "$(git rev-parse HEAD)" --arg config "${CONFIG}" --arg config_hash "${CONFIG_HASH}" --arg command "${COMMAND}" --arg started "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg samples "${SAMPLES}" --arg summary_json "${SUMMARY_JSON}" --arg summary_md "${SUMMARY_MD}" --arg log "${LOG}" '{schema_version:"blind-gains.run-manifest.v1",run_id:$run_id,job_type:"gpu_health_monitor",node:"login",observed_nodes:["an12","an29"],gpu_ids:[],gpu_allocation:[],tensor_parallel_width:0,replica_count:0,placement_justification:"Read-only login-node sampler observes all 16 GPUs and all reported compute processes over SSH; it never sends process signals.",git_hash:$git_hash,config_path:$config,config_hash:$config_hash,data_manifest_hash:$config_hash,seed:null,command:$command,start_time_utc:$started,end_time_utc:null,status:"running",stdout_stderr_log:$log,expected_artifacts:[$samples,$summary_json,$summary_md],duration_seconds:3600,interval_seconds:30,scientific_gate_decision:null,deviations:[]}' > "${MANIFEST}"
