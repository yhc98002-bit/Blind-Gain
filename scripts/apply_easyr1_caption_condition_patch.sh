...
PATCH="${ROOT}/docs/easyr1_caption_condition_patch.diff"
...
if ! grep -q 'IMAGE_CONDITIONS = ' "${TARGET}/verl/utils/dataset.py"; then
  echo "Apply the base image-condition patch first" >&2
  exit 1
fi
if grep -q 'caption_store_paths' "${TARGET}/verl/utils/dataset.py"; then
  echo "EasyR1 caption-condition patch already applied"
  exit 0
fi
...
echo "Applied EasyR1 caption-condition patch"
