...
COMMAND="PYTHONPATH=. .venv/bin/python scripts/watch_m5_merged_relocation.py --run-root ${RUN_ROOT} --archive-root ${ARCHIVE_ROOT} --run-label m5_anchor_longhorizon_400 --evaluation-marker-dir ${MARKER_DIR} --resume-after-step ${RESUME_AFTER} --stop-after-step ${STOP_AFTER}"
...
jq -n --arg run_id "${RUN_ID}" --arg parent "${TRAINING_RUN}" --arg git_hash "$(git rev-parse HEAD)" \
  ...
  --arg markers "${MARKER_DIR}" --arg start "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg log "${LOG}" --argjson resume_after "${RESUME_AFTER}" --argjson stop_after "${STOP_AFTER}" \
  '{schema_version:"blind-gains.run-manifest.v1",run_id:$run_id,job_type:"m5_merged_relocation_watch",
    ...
    expected_artifacts:[],retained_final_step:400,resume_after_step:$resume_after,stop_after_step:$stop_after,
