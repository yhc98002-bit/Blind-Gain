...
CONFIG="configs/eval/m11_generalization_reconciled_backfill_v1.json"
cd "${ROOT}"

CRITICAL_FILES=(
  "${CONFIG}"
  scripts/launch_m11_reconciled_backfill_queue.sh
...
git diff --quiet HEAD -- "${CRITICAL_FILES[@]}" || {
  echo "critical M11 backfill code or config differs from HEAD" >&2
  exit 2
}
for FILE in "${CRITICAL_FILES[@]}"; do
  git ls-files --error-unmatch "${FILE}" >/dev/null 2>&1 || {
    echo "untracked critical M11 backfill file: ${FILE}" >&2
    exit 2
  }
done
...
while IFS= read -r ACTIVE_MANIFEST; do
  if jq -e '(.job_type == "m11_generalization_reconciled_backfill_queue") and (.status == "running")' \
    "${ACTIVE_MANIFEST}" >/dev/null 2>&1; then
    echo "active M11 reconciled backfill queue already exists: ${ACTIVE_MANIFEST}" >&2
    exit 73
  fi
done < <(find experiments/runs -mindepth 2 -maxdepth 2 -type f -name run_manifest.json | sort)

STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_ID="m11_reconciled_backfill_login_${STAMP}"
...
jq -n \
  ...
  '{
    ...
    replica_count: 0,
    placement_justification: "Login-only watchdog reconciles exact immutable M11 cells and backfills independent TP1 Gemma jobs on an29 GPUs 0/1/3/4 without reading performance values.",
    git_hash: $git_hash,
...
kill -0 "$(cat "${RUN_DIR}/pids/login.pid")" 2>/dev/null || {
  echo "M11 reconciled backfill queue exited during startup" >&2
  exit 1
}
printf '%s\n' "${RUN_DIR}"
