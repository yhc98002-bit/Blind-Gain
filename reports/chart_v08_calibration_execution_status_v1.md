# Chart V08 Calibration Execution Status V1
...
- The frozen 100-pair calibration manifest now has structurally complete 3B-real,
  7B-real, and 7B question-blind-caption evaluation lifecycles.
- The frozen 200-row no-star/random-star necessity manifest has a structurally
  complete 7B-real lifecycle.
- Model-performance fields were not opened or interpreted while these plumbing
...
- The 3B caption gate, strong-captioner gate, attacker gate, and necessity-effect
  analysis are pending.
- The caption-generation manifest does not stamp the question-blind prompt hash.
  Every caption row preserves the exact prompt text, so this is recorded as an
  operational provenance deviation rather than repaired in-place.
...
- Run the pending caption and attacker cells on genuinely free permanent-node GPUs.
- Compute the preregistered star-ablation necessity contrast only after the input
  lifecycles and human audit are complete.
...
- `reports/chart_v08_calibration_execution_status_v1.json`.
