...
| ID | Task | Status | Note |
|---|---|---|---|
| DOC | Three PI docs committed verbatim (`docs/` + root `EXPERIMENT_TODO.md`) | pass | Local 2026-08-16T00:29 versions installed at `51060de`; sha256 verified both sides (`e0563bd1…`, `1ee074dc…`, `f14adeae…`). |
| 1a | Storage snapshot: over-quota → `status:"fail"`; guard fails on quota exhaustion instead of retry-forever | pass | Live defect on record: snapshot carried `free_bytes: -461684596736` with `status:"pass"` (Jul-19 loop predated the 2.5 TiB constant). Fixes + `StorageQuotaExhaustedError` (terminal, no retry) + loop restart (`…20260816T084012Z`); snapshot now quota 2748779069440, positive free. I10 fixtures; 33 storage tests pass. |
| 1b | Waiter wedged-vs-dead: alive-but-stalled trainer pauses the deadline clock; deadlines abort only dead chains | pass | `scripts/chain_wait_helper.py` (active deadline, WEDGED visible, dead-stop after 2 confirmations, fail-closed alive); both canonical waiters retrofitted. 6-test I10 fixture (`tests/test_chain_wait_helper.py`). |
| 1c | E3 STAGE-B argument-order fix | pass | Tracing: the STAGE-B defect was the merge-launcher arity check (`-lt 4`→`-lt 3`), fixed at `da0751d` **without** its I10 fixture; fixture shipped (`tests/test_launch_caption_store_merge_arity.py`, 3 tests). Runner call order matches the launcher contract; no surviving defect found. |
| 2 | Storage rule: delete non-terminal steps not §21-referenced; keep terminal + best + §21-referenced | pass | 854,949,371,942 bytes / 65 step dirs / 20 runs deleted; 39 kept; 1 tracker-less run skipped fail-closed; m7/ + lh2/ untouched. Record in `reports/storage_cleanup_20260814.md`; after: used 1.217 TB, free 1.531 TB. `scripts/apply_storage_retention_rule_20260816.py`. |
| 3 | E4 wording: recompute UNfolded per-attacker AUC CIs; criterion untouched | pass | v1 folded numbers reproduced exactly (all attackers). Literal "CI includes 0.5": all per-template scopes yes; pooled dinov2 [0.508, 0.553] and pooled frequency_stat [0.516, 0.576] no; metadata yes everywhere. Folded gate verdict unchanged (PASS). `reports/track4_premise_v2_e4_wording_resolution_v1.*`; per-item scores banked (1,920 rows). |
| 4 | E2 answer-balance: registered constraint + one-shot regeneration of the five failing types (+ approved branch-(c) n=5); E1/E2/E3 re-run on v2 | pass | dev_v2: 160 groups, balance ≤ 0.10/value (k 13–14), n=5 easy types, disjoint, verifier clean. E1 **PASS** (0.5125 in band; branch (a); n=5 frozen). E2 **FAIL** all five (0.15–0.20): tier-1 lenient matcher scores gold `1` vs constant `-1` — a lenient-class collision, per-value cap held; exclusions stand; revision fork → PI. E3 **PASS ×5 both readings**; `chained_premise_easy` 0.2000 ≤ 0.233 resolves under reading (a). `reports/track4_premise_v2_{dev_v2_build,gate_readout_v2,e3_readout_v2}*`. |
| 5 | Two-seed R3 readout | pass | Both 08-15 eval failures diagnosed and fixed (launcher env-assignment bug + malformed plain-text claim from the LH2 chain); evals relaunched 08:25/08:26Z, landed 2026-08-16 (4,239 rows each). Registered two-seed estimator complete: recovery A2-gray 0.7265 [0.6592, 0.7961] / A2b 0.7132 [0.6483, 0.7844] / A3 0.8840 [0.8091, 0.9632]; geo3k differences +0.6476/+0.5948, directions hold; ρ_gain < 0 all blind arms; 0/5000 undefined draws in all nine eligible intervals. `reports/m7_r3_readout_v2.*`. |
| 6 | ST3-7B registration draft (no launch) | pass | `docs/registered_stage3_7b_v1.md`, PAPER2 §5 quoted verbatim; launch gates: two-seed R3 + HB P2 gates + PI merge. DRAFT, unsigned ratification line. |
| 7 | M13 paper-facing numbers table from §21 | pass | `reports/paper1_numbers_table_v1.md`; frozen slots (two-seed R3, LH2 direction, C6 tier) marked; §21 gaps surfaced (F1b, F2, F4, F5, F6-T1, F7 rowless). |
...
| R3md | `build_m7_r3_readout.py` md renderer: bootstrap-nested CIs rendered (previously "[NA]" while the JSON carried them) | pass | `_fmt_ci` fixed + I10 fixture; seed-1 MD golden re-pinned with the pre-fix hash retained in-comment (JSON golden untouched — arithmetic byte-identical); `reports/m7_r3_readout_v2.md` re-rendered from the unchanged payload, old md archived as `.pre_ci_fix_20260816`. 31 R3 fixture tests pass. |
| HBP0.2 | Cue-ladder retraction headers (08-12 dispatch P0.2) | pass | Banners applied to `reports/cue_ladder_readout_v1.md` and RESULTS §16 (supersede, not delete); neither carried one before. HB build ledger opened: `reports/hier_benchmark_progress.md`. |
| LH2r | LH2 seg1 recovery (deviation) | pass | 08-15 relaunch died at ~20 min: resume-safe logger guard refused three stale 2026-08-06 logger artifacts in a stage dir with zero banked boundaries; orphaned `running` manifest. Recovery: archived aside (never deleted), orphan manifest closed post-hoc (`fail`/exit 1), chain relaunched from the immutable copy; gates passed 2026-08-16T17:22:33Z, **seg-1 training since 17:22:36Z** (`lh2_seed2_seg1_an12_20260816T172233Z`, an12 0–3). |
